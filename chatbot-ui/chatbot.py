"""
Chatbot RAG Phòng Ban
======================

Ứng dụng Chatbot đơn giản sử dụng department_info_tool để truy xuất thông tin về phòng ban
và các task trong công ty, với tích hợp LLM cho xử lý ngôn ngữ tự nhiên.

Yêu cầu:
- Python 3.8+
- Streamlit
- LLM API (Qwen hoặc tương đương) chạy ở http://localhost:1234/v1
...
Sử dụng:
1. Nhập câu hỏi về một phòng ban trong ô nhập liệu
2. Ứng dụng sẽ tự động phát hiện phòng ban từ câu hỏi
3. Thông tin về phòng ban và các task sẽ được hiển thị qua LLM
"""
...
Danh sách phòng ban hiện có trong hệ thống:
- 2D
- Dự toán
- Khách hàng
- Kinh doanh
...
LƯU Ý: "Marketing và Sales" KHÔNG phải là tên phòng ban mà là tên giai đoạn MKT-SALES.
Nếu người dùng hỏi về "Marketing và Sales", hãy hiểu đó là hỏi về giai đoạn MKT-SALES hoặc các phòng ban liên quan như Marketing và Kinh doanh.
...
"""
    ...
    # Thêm thông tin về phòng ban và giai đoạn con nếu có
    additional_info = ""
    if department:
        additional_info += f"\nPhòng ban cụ thể: {department}"
    if sub_phase:
        additional_info += f"\nGiai đoạn con cụ thể: {sub_phase}"
    
    if additional_info:
        additional_info += "\nHãy tập trung vào thông tin liên quan đến phòng ban và giai đoạn con này."
...
7. SỬ DỤNG LỊCH SỬ HỘI THOẠI:
   - Nếu câu hỏi liên quan đến hội thoại trước đó, hãy tham khảo lịch sử để cung cấp câu trả lời nhất quán
   - Nếu câu hỏi hiện tại không đề cập phòng ban cụ thể, nhưng hội thoại trước có đề cập, hãy hiểu rằng người dùng vẫn đang hỏi về phòng ban đó
   - Nếu câu hỏi hiện tại giống như "còn gì nữa không?", "tiếp theo là gì?", hãy tiếp tục trình bày về phòng ban đã đề cập trong hội thoại trước
8. BỐI CẢNH HỘI THOẠI:
   - Nếu trong lịch sử hội thoại, người dùng có hỏi về phòng ban {dept_info['department']} và giai đoạn cụ thể, và câu hỏi hiện tại hỏi tiếp về giai đoạn đó, hãy tập trung trả lời về vai trò của phòng ban {dept_info['department']} trong giai đoạn đó
   - Khi người dùng dùng từ "họ", "nhân viên ở đây", "bộ phận này" mà không chỉ rõ phòng ban, hiểu rằng đang đề cập đến phòng ban {dept_info['department']}
...
def query_llm(prompt: str, system_prompt: str, max_tokens=16000, stream=True) -> str:
    ...
    try:
        ...
        payload = {
            ...
            "stream": stream  # Sử dụng giá trị tham số stream
        }
...
def handle_general_query(query: str, use_llm=True, session_id: Optional[str] = None) -> str:
    ...
    phases_info = {
        ...
        "PROPOSAL": {
            ...
            "sub_phases": ["Proposal"],
            "departments": ["Kinh doanh", "Dự toán", "Thiết kế", "Team dự án", "Khách hàng"]
        },
        "CONSTRUCTION": {
...
6. Khi người dùng hỏi về tất cả giai đoạn của tất cả phòng ban/ công ty thì đó là câu hỏi general.
...
### Ví dụ phân loại:
1. "Phòng kinh doanh có những công việc gì?" -> {"department": "Kinh doanh", "query_type": "department_specific", "error": false}
...
8. "Họ có tham gia giai đoạn 1 không?" (sau khi hỏi về Thi công) -> {"department": "Thi công", "query_type": "department_specific", "error": false}
9. "Có các bước nào trong giai đoạn này?" (sau khi hỏi về phòng Marketing) -> {"department": "Marketing", "query_type": "department_specific", "error": false}
10. "Giai đoạn Construction có những task gì?" (sau khi hỏi về phòng Thi công) -> {"department": "Thi công", "query_type": "department_specific", "error": false}
11. "Bộ phận này làm gì trong giai đoạn PROPOSAL?" (sau khi hỏi về Kinh doanh) -> {"department": "Kinh doanh", "query_type": "department_specific", "error": false}
"""
...
Nếu câu hỏi hiện tại là tiếp nối câu hỏi trước và không đề cập rõ phòng ban, 
hãy sử dụng phòng ban từ lịch sử hội thoại gần đây.

QUAN TRỌNG NHẤT: CHỈ TRẢ VỀ JSON, KHÔNG THÊM BẤT KỲ THÔNG TIN NÀO KHÁC. KHÔNG GIẢI THÍCH. KHÔNG THÊM THẺ <THINK>.
"""
